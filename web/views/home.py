...
def index(request, *args, **kwargs):
    ...
    if kwargs:
        ...
        #url  name的别称 在html与view的用法不同  如果没有定义名称 这里传入的是 args=args
...
def filter(request, site, condition, val):
    ...
    if condition == 'tag':
        ...
        article_list = models.Article.objects.filter(blog=blog).extra(
            #%%的目的是输出%  目的是在数据库输出%Y-%m
            #strftime  是django 内置的  mysql应该用'date_format(create_time,"%%Y-%%m")=%s'
